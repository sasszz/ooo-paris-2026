...
async function loadCheckins() {
  try {
    ...
    console.warn("checkins load failed");
  }
...
async function saveCheckins(list) {
  ...
  try {
    ...
    console.warn("checkins save failed");
  }
}
...
function renderMap(checkins) {
  ...
  geo.forEach((c, i) => {
    ...
    const cap = c.caption
      ? `<div class="popup-cap">${c.caption.length > 70 ? c.caption.slice(0, 70) + "…" : c.caption}</div>`
      : "";
  ...
  const cities = new Set(
    geo
      .map((c) => {
        const p = c.location.split(", ");
        return p.length >= 3 ? p[p.length - 2].trim() : null;
      })
      .filter(Boolean),
  );
  const countries = new Set(
    geo.map((c) => c.location.split(", ").pop().trim()),
  );
  const stats = {
    stops: geo.length,
    cities: cities.size,
...
async function renderPage() {
  ...
  _captionEl.textContent = latest.caption || "";
  _captionEl.style.display = latest.caption ? "" : "none";
  document.getElementById("checkin-time").textContent = fmtLong(
    latest.timestamp,
  );
  const croissantEl = document.getElementById("checkin-croissants");
  if (latest.croissants) {
    croissantEl.textContent = "🥐".repeat(latest.croissants);
    croissantEl.style.display = "block";
  } else {
    croissantEl.style.display = "none";
  }
  ...
  if (checkins.length > 1) {
    ...
    const showMore = (from) => {
      ...
      pastCheckins
        ...
        .forEach((c) => list.appendChild(makeHistoryItem(c)));
      ...
      if (from + PAGE < pastCheckins.length) {
        ...
        btn.onclick = () => {
          const extras = [...list.querySelectorAll(".history-item")].slice(
            PAGE,
          );
...
async function postCheckin() {
  ...
  if (!location) {
    status.innerHTML =
      '<span class="status-err">Please enter a location name.</span>';
    return;
  }
  ...
  if (fileInput.files[0]) {
    if (fileInput.files[0].size > 5 * 1024 * 1024) {
      status.innerHTML =
        '<span class="status-err">Photo must be under 5MB.</span>';
      btn.disabled = false;
      return;
    }
    try {
      ...
      status.innerHTML =
        '<span class="status-err">Photo upload failed. Try again.</span>';
      btn.disabled = false;
      return;
    }
  }
  ...
  const croissants =
    parseInt(document.getElementById("croissant-count").textContent) || 0;
  const entry = {
    location,
  ...
  setTimeout(() => {
    closeAdmin();
    ["post-location", "post-lat", "post-lng", "post-caption"].forEach(
  ...
  function showSuggestions(items) {
    ...
    items.forEach((item, i) => {
      ...
      li.addEventListener("mousedown", (e) => {
        ...
        document.getElementById("post-lat").value = parseFloat(
          item.lat,
        ).toFixed(6);
        document.getElementById("post-lng").value = parseFloat(
          item.lon,
        ).toFixed(6);
        list.classList.remove("open");
      });
...
async function uploadToCloudinary(file) {
  ...
  const r = await fetch(
    "https://api.cloudinary.com/v1_1/dpf8t8yta/image/upload",
    {
      method: "POST",
      body: form,
    },
...
function openLightbox(c) {
  ...
  const croissantHtml = c.croissants
    ? `<span class="lightbox-croissant-inline">🥐 × ${c.croissants}</span>`
    : "";
...
function openLightboxByTs(ts) {
  openLightbox(_checkinByTs[ts]);
}
...
(function () {
  ...
  function flipCard(card, oldVal, newVal) {
    ...
    setTimeout(() => {
      card.querySelector(".flip-upper span").textContent = newVal;
    }, 280);
    setTimeout(() => {
      card.querySelector(".flip-lower span").textContent = newVal;
    }, 560);
  }
...
function openCroissantPopup() {
  let idx;
  do {
    idx = Math.floor(Math.random() * CROISSANT_MSGS.length);
  } while (idx === _lastCroissantMsg);
  _lastCroissantMsg = idx;
  document.getElementById("croissant-popup-msg").textContent =
    CROISSANT_MSGS[idx];
  ...
  setTimeout(() => {
    _croissantPopupJustOpened = false;
  }, 300);
}
function enableMotion() {
  ...
  DeviceOrientationEvent.requestPermission()
    .then((s) => {
      if (s === "granted" && window._jarListen) window._jarListen();
    })
    .catch(() => {});
}
function skipMotion() {
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") closeCroissantPopup();
});
...
async function dropCroissant() {
  if (_godMode) {
    ...
    const count = jarBody.querySelectorAll(".jar-croissant").length;
  ...
  span.addEventListener(
    "animationend",
    () => span.classList.remove("jar-croissant-new"),
    { once: true },
  );
  const jarBody = document.getElementById("jar-body");
  jarBody.appendChild(span);
...
function toggleGodMode() {
  ...
  if (_godMode) {
    ...
    btn.textContent = "✦ exit god mode";
    jarBody.classList.add("god-mode");

    jarBody.innerHTML = "";
    if (_jarBodies === null) _jarBodies = [];
    _jarBodies.length = 0;
    updateJarCount(0);
    ...
    if (_jarBodies) {
      const W = jarBody.clientWidth || 110,
        S = 18;
      _jarBodies.length = 0;
      [...jarBody.querySelectorAll(".jar-croissant")]
        .slice(-30)
        .forEach((el) => {
...
let _jarGX = 0,
  _jarGY = 0;
let _jarLastTilt = Date.now(),
  _jarPrevGamma = 0,
  _jarPrevBeta = 90,
  _jarHasOrientation = false;
...
function beginJarPhysics() {
  ...
  const W = body.clientWidth,
    H = body.clientHeight;
  if (W < 10 || H < 10) {
    requestAnimationFrame(beginJarPhysics);
    return;
  }
  const S = 18;
  const MAX_BODIES = 30;
  _jarBodies = [...body.querySelectorAll(".jar-croissant")]
    .slice(-MAX_BODIES)
    .map((el) => {
...
function jarPhysicsFrame() {
  ...
  const W = body.clientWidth,
    ...
    S = 18;
  const tiltActive =
    _jarHasOrientation && (Math.abs(_jarGX) > 0.15 || _jarGY < 0.85);
  const idleMs = Date.now() - _jarLastTilt;
  const energy = _godMode
    ? 1
    : tiltActive
      ? 1
      : idleMs < 3000
        ? 1
        : Math.max(0, 1 - (idleMs - 3000) / 5000);
  const WANDER = 0.0015 * energy,
    TILT = 0.018;
  ...
  _jarBodies.forEach((b) => {
    ...
    if (b.x > W - S - WALL_R)
      b.vx -= ((b.x - (W - S - WALL_R)) / WALL_R) * WALL_F;
    if (b.y < WALL_R) b.vy += ((WALL_R - b.y) / WALL_R) * WALL_F;
    if (b.y > H - S - WALL_R)
      b.vy -= ((b.y - (H - S - WALL_R)) / WALL_R) * WALL_F;
  });
...
function initJarTilt() {
  const listen = () => {
    window.addEventListener("deviceorientation", (e) => {
      ...
      if (
        Math.abs(gamma - _jarPrevGamma) > 0.5 ||
        Math.abs(beta - _jarPrevBeta) > 0.5
      ) {
        _jarLastTilt = Date.now();
        _jarPrevGamma = gamma;
        _jarPrevBeta = beta;
